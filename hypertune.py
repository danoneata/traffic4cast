...
class PyTorchWorker(Worker):
    ...
    def get_configspace():
        ...
        lr = CSH.UniformFloatHyperparameter('optimizer_lr', lower=1e-6, upper=1e-1, default_value='1e-2', log=True)
    
        cs.add_hyperparameters([lr])
        return cs 

def main():
    parser = argparse.ArgumentParser(description='Parallel execution of hyper-tuning')
    ...
    if args.worker:
        ...
        w = PyTorchWorker("Berlin","temporal-regression-speed-12",
                            run_id="t4cast",id=0, nameserver='127.0.0.1')
    ...
    # Run and optimizer
    bohb = HyperBand(configspace = PyTorchWorker.get_configspace(), # model can be an arg here?
                        run_id = 't4cast',
